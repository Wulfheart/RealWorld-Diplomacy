...
reslv_moves(Orderlist orders[], bool ign_dislodge)
{
    ...
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
    {
		...
		else if (gtype(gp) == O_TRAFO && !gresolved(gp))
		{
			setgresolution(gp, R_SUCCESS);
		}
    }
...
cutsupports ()
{
    register GraphNode gp, gp1;
	Canal c;
	Order op;

    /* cut supports from all moves except convoyed armies
	and canal using fleets and for attacks against us */
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
    {
		op = gorder(gp);
	  	if (gtype(gp) == O_MOVE && !gfails(gp) && !convoyed(gp)
		  && !canalmove(gp)                   /* not a move though a canal */
		  ...
		  && !ToSuez(ounit(op), ostart(op), oscoast(op), ofinish(op)) /* not a move to Suez region */ 
		  && !FromSuez(ounit(op), ostart(op), oscoast(op), ofinish(op)) /* not a move from Suez region */ 
	      ) 
		{  
	...
	of not moving canal controllers and of targets of
	moves around Africa                               */
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp)  
			/* a move that has not yet failed */
	  	  && (((gtype(gdest(gp)) == O_CONV) && !gfails(gdest(gp)))
		    /* is trying to attack an as yet convoing fleet */
		  && stronger(gp, gdest(gp))))
		    /* and might dislodge it */
		{
	       setgresolution(gdest(gp), R_MAYBE);
		}
		else if (gtype(gp) == O_MOVE && !gfails(gp)  
			/* a move that has not yet failed */
	  		&& iscanalcenter(gsector(gdest(gp))) 
				&& (gCanalPermissionMode == VCP_HOLD 
					|| gCanalPermissionMode == VCP_STAY)
				&& !gfails(getcanal(gsector(gdest(gp))))
			/* and a canal controller that has to stay */
	      && stronger(gp, gdest(gp))) 
		    /* and might dislodge it */
		{
	       setgresolution(getcanal(gsector(gdest(gp))), R_MAYBE);
		}
		else if (gtype(gp) == O_MOVE && !gfails(gp)  
			/* a move that has not yet failed */
	  		&& iscanalcenter(gp) 
				&& (gCanalPermissionMode == VCP_STAY)
				&& !gfails(getcanal(gsector(gp))))
			/* and is a canal controller that has to stay */
	    {
	       setgresolution(getcanal(gsector(gp)), R_MAYBE);
		}
		else if (gtype(gp) == O_MOVE && !gfails(gp)  
			/* a move that has not yet failed */
	  	  && (((gtype(gdest(gp)) == O_SUPP) && !gfails(gdest(gp)))
		    /* is trying to cut a support */
		  && (isconvtosuez(gp) 
				|| FromSuez(ounit(gorder(gp)), ostart(gorder(gp)), oscoast(gorder(gp)), ofinish(gorder(gp)))	
				|| ToSuez(ounit(gorder(gp)), ostart(gorder(gp)), oscoast(gorder(gp)), ofinish(gorder(gp))))
			/* and is going around Africa */
		  && stronger(gp, gdest(gp))))
		    /* and might dislodge it */
		{
	       setgresolution(gdest(gp), R_MAYBE);
	...
	/* check the convoyed armies and canal using fleets */
    clearmarks();
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp)
		...
		else if (gtype(gp) == O_MOVE && !gfails(gp) && canalmove(gp))
		{
		   /* if the unit controlling the canal center belonging 
			  to the canal is marked as a possibly disloged 
			  or possibly leaving unit */
		   if (gresolution(getcanal(gsector(gp)))
			          == R_MAYBE)
				{
					setgresolution(gp, R_MAYBE);
				}
	...
	/* cut supports by convoyed armies and canal using fleets that *will* get there */
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp) && convoyed(gp)
	      && gtype(gdest(gp)) == O_SUPP && !gfails(gdest(gp))
		  ...
		  && (!isconvtosuez(gp) || follow(gp))
		  && ((gtype(gdest(gdest(gp))) != O_CONV)
				/* not supporting attack against convoying unit */		    
			  //&& gtype(gspec(gdest(gp))) != O_CONV)
				/* Rule XII.5 from 1982 rules */			
				|| gSzykman 
				/* ignore Rule XII.5*/
		  ...
		  /* || stronger(gp, gdest(gp))) */
		{
		  ...
		  && ((!ToSuez( ounit(gorder(gp)), ostart(gorder(gp)), oscoast(gorder(gp)), ofinish(gorder(gp))) 
				&& !FromSuez( ounit(gorder(gp)), ostart(gorder(gp)), oscoast(gorder(gp)), ofinish(gorder(gp)) )) 
					|| follow(gp)))
		  /* not an unsuccessful move from or to Suez region */ 
	       /* || stronger(gp, gdest(gp))) */
		{
		  if (gSzykman)
		  {
			for (gp1 = startnode(); isnode(gp1); nextnode(gp1)) 
			{
				// the following procedure does not work as desired,
				// since "follow" already assigns resolution info to graph nodes
				/*
				if (gtype(gp1) == O_MOVE && !gfails(gp1) 
					&& gsector(gp1) == ccenter(getcanal(gsector(gp))) 
					&& !gfails(getcanal(gsector(gp1)))
					&& !follow(gp1))
					// an as yet unsuccessful move from the canal center 
				{
					setgresolution(gdest(gp), R_CUT);
					decgstrength(gdest(gdest(gp)));
					// if this would make the move a success 
					if (follow(gp1))
					{
						// a paradox occurs 
						setgresolution(getcanal(gsector(gdest(gp))), R_PARADOX);
						setgresolution(gp, R_PARADOX);
						// undo cut 
						setgresolution(gdest(gp), R_NONE);
						incgstrength(gdest(gdest(gp)));
						break;
					}
					// undo cut for other checks 
					setgresolution(gdest(gp), R_NONE);
					incgstrength(gdest(gdest(gp)));
				}
				*/
				...
				decgstrength(gdest(gdest(gp)));
				if (gtype(gp1) == O_MOVE && !gfails(gp1) 
					&& gsector(gdest(gp1)) == ccenter(getcanal(gsector(gp))) 
					&& !gfails(getcanal(gsector(gdest(gp1))))
					&& follow(gp1))
					/* a successful move into the canal center */
				{
					/* a paradox occurs */
					setgresolution(getcanal(gsector(gdest(gp))), R_PARADOX);
					setgresolution(gp, R_PARADOX);
	       ...
	       /* || stronger(gp, gdest(gp))) */
		{
		  if (gSzykman)
		  {
			...
			decgstrength(gdest(gdest(gp)));
			if (!follow(gp))
				/* the cut causes the move to fail */
			{
				/* a paradox occurs */
				setgresolution(gp, R_PARADOX);
				/* undo cut */
				setgresolution(gdest(gp), R_PARADOX);
				incgstrength(gdest(gdest(gp)));
				break;
			}
		  }
	...
	/* a routine to deal with leaving canal controllers is still missing */
    /* clear the "maybes" */
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
	...
	for (c = startcanal(); iscanal(c); nextcanal(c))
		if (gresolution(c) == R_MAYBE)
		   setgresolution(c, R_NONE);
    ...
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
    {
		if (gtype(gp) == O_MOVE && !gfails(gp) && convoyed(gp)
	      ...
	      /* || stronger(gp, gdest(gp))) */
		{
			if (gSzykman && !canconvoy(gp, gp)) 
			{
				/* cut regardless of convois first */
				setgresolution(gdest(gp), R_CUT);
				decgstrength(gdest(gdest(gp)));
				if (!canconvoy(gp, gp))		
				{
			...
			else if (gtype(gdest(gdest(gp))) != O_CONV 
						&& gtype(gspec(gdest(gp))) != O_CONV)
						/* Rule XII.5 */
			{
				/* do it like the Rule suggests */
				setgresolution(gdest(gp), R_CUT);
				decgstrength(gdest(gdest(gp)));
			}
		}
	...
	/* mark dislodges of canal controllers */
	for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp) 
		  && iscanalcenter(gsector(gdest(gp)))
		  && !gfails(getcanal(gsector(gdest(gp))))
		  && (gCanalPermissionMode == VCP_HOLD)
		  && gtype(gdest(gp)) != O_MOVE
		  && follow(gp))
		  {
				setgresolution(gdest(gp), R_DISLODGE);
				setgresolution(getcanal(gsector(gdest(gp))), R_CANAL);
		  }
	}

	/* check canal using fleets */
	clearmarks();
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
	  	if (gtype(gp) == O_MOVE && !gfails(gp) && canalmove(gp)
			&& (gfails(getcanal(gsector(gdest(gp))))))
				{				
					setgresolution(gp, R_CANAL);
				}
	}
    ...
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp) 
		  ...
		  && strongest(gp))
		  // strongest added by Dirk Brueggemann. 
	...
	/* cut all transformation orders */
    for (gp = startnode(); isnode(gp); nextnode(gp)) 
	{
		if (gtype(gp) == O_MOVE && !gfails(gp) 
	      && gtype(gdest(gp)) == O_TRAFO && !gfails(gdest(gp)))
		{
	       setgresolution(gdest(gp), R_FAILS);
	    }
	}
     ...
     if (gtype(reg2) == O_MOVE)
	  if (gdest(reg1) == gdest(reg2) 
	      || (gdest(reg2) == reg1 && gdest(reg1) == reg2))
	       ...
	       return (gstrength(reg1) > basestrength);
     else
	  return (gstrength(reg1) > gstrength(reg2));
}
	...
	if (gtype(reg2) == O_MOVE)
	{
		if (gdest(reg1) == gdest(reg2) 
	      || (gdest(reg2) == reg1 && gdest(reg1) == reg2))
	       ...
	       return (strength_noselfsupp(reg1) > basestrength);
	}
	else
	{
		return (strength_noselfsupp(reg1) > gstrength(reg2));
     ...
     for (gp = startnode(); isnode(gp); nextnode(gp)) 
     {
	      	  ...
	      	  gresolution(gdest(gp)) == R_CUT_DIS))
	    {
	       ...
	       while (isneighbor(lp)) 
	       {
				...
				/* and it's not a unit trying to use an isthmus */
					&& !isisthmus(lp)
				/* and it's not a unit trying to hop */
					&& !isjump(lp)
				/* and it's not a unit trying to use a canal */
					&& !(issamecanal(gsector(gp),getsector(map, nsector(lp))) && !gAllowsCanalRetreats)
				/* and it's not a wing trying to use a canal if this is forbidden */
					&& !(utype(up) == U_WNG && issamecanal(gsector(gp),getsector(map, nsector(lp))) && !gAllowsCanalWingUsage)
				/* and it's not a non wing trying to retreat to a wing only sector */
					&& (utype(up) == U_WNG || island(getsector(map, nsector(lp))) || issea(getsector(map, nsector(lp)))))
			 		addneighbor(&retreats, nsector(lp), ncoast(lp), nspecial(lp));
		    	nextneighbor(lp);
	       }
	       if (utype(up) == U_WNG) 
		   {
				...
				while (isneighbor(lp)) 
				{
		    		...
		    		/* and there hasn't been a bounce there */
						&& !(gbounce(getnode(nsector(lp))))
						&& !(season == S_FALL && isice(getsector(map, nsector(lp))))
						&& !innlist(retreats, nsector(lp), C_NONE, TRUE)
					/* and it's not a unit trying to hop */
						&& !isjump(lp)
					/* and it's not a wing trying to use a canal without permission */
						&& !(issamecanal(gsector(gp),getsector(map, nsector(lp))) && (!gAllowsCanalRetreats || !gAllowsCanalWingUsage)))
			 			addneighbor(&retreats, nsector(lp), C_NONE, nspecial(lp));
		    		nextneighbor(lp);
				}
		        lp = startneighbor(coast2neigh(destp));
				while (isneighbor(lp)) 
